...
[前回](http://rpgp1.github.io/2013-12-17/DXRubyでRPGを作る/ "[2013/12/17]DXRubyでRPGを作る")はRPGに何が必要なのかを考えました。今回も引き続き、DXRubyでRPGを作る方法を模索します。
...
module Event
  class EBook
    class EPage
      attr_reader :args #どんなEPageかを表します。具体的には、add_eventやhold時の引数の数です。
      
      def initialize(arg_num,hold_args)
        #arg_numは発生源を特定するための情報の数
        #hold_argsはhold時に送信する情報量(arg_numを除く)の数値の配列([最低限, 最大数])
        
        @event = {} #イベント保存用ハッシュ
        hold_arg_num = hold_args.map{|i| (i < 0 ? 0 : i.div(1))} #hold_argsの中身に不正なものが無いかの修正
        @args = [arg_num, hold_arg_num.min, hold_arg_num.max] #これを見ればどんなEPageか特定できます
        ...
        tmp.class_eval do
          ...
          if hold_arg_num.min != hold_arg_num.max
            define_method(:hold){|*args|
              ...
              if args.size - arg_num == 0
                ...
                @event[args[0...arg_num]].call(*args[arg_num...args.size]) if @event.has_key?(args[0...arg_num])
              end
            }
          else
          ...
          define_method(:add_event){|*args, &block|
            if args.size != arg_num
              raise ArgumentError, "wrong number of argument (#{args.size} for #{arg_num})", caller(1)
            end
            if block
              @event[args] = block
            else
    ...
    class EPage
      ...
      def initialize(arg_num,hold_args)
        ...
        tmp.class_eval do
          if hold_arg_num.min != hold_arg_num.max
            define_method(:hold){|*args|
              raise ArgumentError, "wrong number of argument (#{args.size} for #{arg_num + hold_arg_num.min}..#{arg_num + hold_arg_num.max})", caller(1) if args.size < arg_num + hold_arg_num.min || args.size > arg_num + hold_arg_num.max
              if args.size - arg_num == 0
                @event[args].call if @event.has_key?(args)
              else
                @event[args[0...arg_num]].call(*args[arg_num...args.size]) if @event.has_key?(args[0...arg_num])
            ...
            if hold_arg_num.min == 0
              define_method(:hold){|*args|
                raise ArgumentError, "wrong number of argument (#{args.size} for #{arg_num})", caller(1) if args.size != arg_num
                @event[args].call if @event.has_key?(args)
              }
            else
              define_method(:hold){|*args|
                raise ArgumentError, "wrong number of argument (#{args.size} for #{arg_num + hold_arg_num.min})", caller(1) if args.size != arg_num + hold_arg_num.min
                @event[args[0...arg_num]].call(*args[arg_num...args.size]) if @event.has_key?(args[0...arg_num])
              }
            end
          end
...
```
  ...
  Event
    .book(name)
      nameの名前でEventモジュールにメソッドを作成し、新しく生成したEBookオブジェクトを返す。
  ...
  EBook
    ...
    #page(name, arg_num, hold_arg_num)
      nameの名前でselfにメソッドを作成し、arg_num,hold_arg_numを渡して新しく生成したEPageオブジェクトを返す。
      
      [PARAM] name:
        定義するメソッド名をStringかSymbolで指定する。
      
      [PARAM] arg_num:
        イベントフォームで、イベント発生源を指定するために必要な情報量を数値で指定する。
        EPage#add_event, #holdの引数の数に影響する。
      
      [PARAM] hold_arg_num:
        イベントフォームで、イベント発生時に送信する情報量(arg_numを除く)を数値の配列で指定する。
        EPage#add_event, #holdの引数の数に影響する。
  ...
  EPage
    #add_event(arg1[, arg2[, arg3 ...]]){|[hold_arg1[, hold_arg2 ...]]| ..}
      ...
      このブロックの引数の個数に対してエラーチェックをしないが、適切な個数でないとhold時にエラーが発生することに注意。[[trap:Event]]
      
      [PARAM] arg1[, arg2[, arg3 ...]]:
        EBook#page時に指定したarg_num個の、イベント発生源を指定するために必要な情報を指定する。
    ...
    #hold(arg1[, arg2[, arg3 ...]][, hold_arg1[, hold_arg2 ...]])
      ...
      [PARAM] [arg1[, arg2 ...]]:
        EBook#page時に指定したhold_arg_numの中の(最小の数)個以上(最大の数)個以下の、イベントに送る情報を指定する。
...
Sceneの条件分岐はMap/Battle/Animationそれぞれのモジュール/クラスの方で.updateを定義しておけば簡単です。今回はコードだけ貼っておきます。
...
module Scene
  ...
  def self.update
    @@scene.__send__(:count_frame) if (@@scene.methods + @@scene.private_methods).include?(:count_frame)
    
    old_scene = @@scene
    
    @@scene.__send__(:update) if (@@scene.methods + @@scene.private_methods).include?(:update)
    
    if @@scene != old_scene
      old_scene.__send__(:quit) if (old_scene.methods + old_scene.private_methods).include?(:quit)
      @@scene.__senf__(:init) if (@@scene.methods + @@scene.private_methods).include?(:init)
    end
    
    old_scene = @@scene
    
    @@scene.__send__(:draw) if (@@scene.methods + @@scene.private_methods).include?(:draw)
    
    if @@scene != old_scene
      old_scene.__send__(:quit) if (old_scene.methods + old_scene.private_methods).include?(:quit)
      @@scene.__send__(:init) if (@@scene.methods + @@scene.private_methods).include?(:init)
    end
  end
end
{% endhighlight %}
